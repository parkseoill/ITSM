...
function fMoveList(){
	var objForm = document.getElementById("menuCreatManageForm");
	objForm.action = "<c:url value='/sym/mnu/mcm/EgovMenuCreatManageSelect.do' />";
	objForm.submit();
}
...
<!-- 검색조건 유지 -->
<input type="hidden" name="searchCondition" value="<c:out value='${resultVO.searchCondition}'/>"/>
<input type="hidden" name="searchKeyword" value="<c:out value='${resultVO.searchKeyword}'/>"/>
<input type="hidden" name="pageIndex" value="<c:out value='${resultVO.pageIndex}' default='1' />"/>

<div class="board">
	<h1>권한별메뉴생성관리 > 메뉴생성</h1><!-- 메뉴생성 -->
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
		<ul>
			<li>
				...
				<span class="btn_b"><a href="#LINK" onclick="fMoveList(); return false;" title="목록">목록</a></span><!-- 목록 이동 -->
			</li>
		</ul>
	</div>
