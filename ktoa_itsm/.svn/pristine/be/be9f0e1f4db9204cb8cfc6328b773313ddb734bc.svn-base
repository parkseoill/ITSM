...
<%--
 /**
  * @Class Name : BizSytmMngUpdt.jsp
  * @Description : 업무시스템 수정 화면
...
function selectList(){
	var objForm = document.getElementById("bizSytmMngForm");
	objForm.action = "<c:url value='/opm/biz/BizSytmMngList.do' />";
	objForm.submit();
}
...
<form:form commandName="bizSytmVO" id="bizSytmMngForm" name="bizSytmMngForm" method="post" action="${pageContext.request.contextPath}/opm/biz/updateBizSytmMngUpdt.do">
	<!-- 검색조건 유지 -->
	...
	<div class="wTableFrm">
		...
		<table class="wTable">
			...
			<tr>
				...
				<td class="left">
				    <form:input id="bizSytmCd" path="bizSytmCd" maxlength="30" title="시스템코드" readonly="true" class="readOnlyClass" cssStyle="width:150px" />
		...
		<!-- 하단 버튼 -->
		<div class="btn">
			<input class="s_submit" type="submit" value='<spring:message code="button.update" />' onclick="updateBizSytmMng(); return false;" /><!-- 수정 -->
