	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<input id="F1" class="s_input2 vat" name="searchKeyword" type="text"  value='<c:out value="${searchVO.searchKeyword}"/>'  size="80" maxlength="60" title="검색조건" />
	...
	<table class="board_list">
		...
		<tbody>
			<c:forEach var="result" items="${list_menumanage}" varStatus="status">
			  <tr onclick="selectMenuCreat('<c:out value="${result.authorCode}"/>'); return false;">
