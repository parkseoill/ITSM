...
function fn_DeleteList() {
    let checkProgrmFileNms = "";
    let checkedCount = 0;
    
    $("input[name=checkField]").each(function() {
    	if($(this).is(":checked") == true) {
    		checkProgrmFileNms += checkedCount==0? $(this).val() : "," + $(this).val();
    		checkedCount++;
    	}
    });
    ...
    if(confirm("<spring:message code="common.delete.msg" />")){	//삭제하시겠습니까?
    	...
    	$("#progrmManageForm").attr("action", "<c:url value='/sym/prm/EgovProgrmManageListDelete.do'/>");
...
<div class="board">
	...
	<form name="progrmManageForm" id="progrmManageForm" action ="<c:url value='/sym/prm/EgovProgramListManageSelect.do' />" method="post">
			 ...
			 <c:forEach var="result" items="${list_progrmmanage}" varStatus="status">
			  <tr onclick="selectUpdtProgramListDetail('<c:out value="${result.progrmFileNm}"/>'); return false;">
			    <td onclick="event.cancelBubble=true">
			       <input type="checkbox" name="checkField" class="check2" title="선택" value="<c:out value='${result.progrmFileNm}'/>">
			    </td>
			    <td title="<c:out value="${result.progrmFileNm}"/>">
		            <span class="link">
			    ...
			    <td title="<c:out value="${result.URL}"/>">
