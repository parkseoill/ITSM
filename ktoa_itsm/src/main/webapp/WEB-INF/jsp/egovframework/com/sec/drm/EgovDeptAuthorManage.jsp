...
function fncSelectDeptAuthorList(pageNo) {
	if(document.listForm.deptNo.value == '') {
...
<div class="board">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
		<ul>
			...
			<!-- 검색키워드 및 조회버튼 -->
			<li>
				<input name="cmpnyNm" type="text" value="<c:out value='${deptAuthorVO.cmpnyNm}' />" size="20" title="회사명" onkeypress="press();" readonly="readonly" /><!-- 회사명 -->
				<input name="deptNo" type="text" value="<c:out value='${deptAuthorVO.deptNo}' />" size="15" title="<spring:message code="comCopSecDrm.list.deptCd" />" onkeypress="press();" readonly="readonly" /><!-- 부서코드 -->
				...
				<input id="deptSelectPopup" type="button" class="s_btn" value="<spring:message code="comCopSecDrm.btn.deptSelectPopup" />" title="부서 선택" /><!-- 부서조회팝업 -->
	...
	<tr>
		<td>
			...
			<input type="hidden" name="checkId" value="<c:out value="${deptAuthor.uniqId}"/>" />
		</td>
		<td><c:out value="${deptAuthor.userId}"/></td>
		<td><c:out value="${deptAuthor.chrgrNm}"/></td>
		<td><c:out value="${deptAuthor.authorCode}"/></td>
		<td>
			<select name="authorManageCombo" title="권한명">
				<c:forEach var="authorManage" items="${authorManageList}" varStatus="status">
					<option value="<c:out value="${authorManage.authorCode}"/>" <c:if test="${authorManage.authorCode == deptAuthor.authorCode || (deptAuthor.regYn == 'N' and authorManage.authorCode == 'ROLE_ANONYMOUS')}">selected</c:if> ><c:out value="${authorManage.authorNm}"/></option>
				</c:forEach>
			</select>
		</td>
...
<input type="hidden" name="cmpnyNo" value="<c:out value='${deptAuthorVO.cmpnyNo}'/>"/>
</form:form>
