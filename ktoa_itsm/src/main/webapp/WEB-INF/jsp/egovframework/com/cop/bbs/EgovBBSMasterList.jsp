...
<div class="board">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
		<ul>
			<li style="padding-right:2%;">
				...
				<input style="vertical-align: middle;" checked="checked" type="radio" name="searchCnd" <c:if test="${searchVO.searchCnd == '0'}">checked="checked"</c:if> value="0">게시판명</input>
				<input style="vertical-align: middle;" type="radio" name="searchCnd" <c:if test="${searchVO.searchCnd == '1'}">checked="checked"</c:if> value="1">게시판 소개내용</input>
			</li>
			<!-- 검색키워드 및 조회버튼 -->
			<li>
				<label for="">검색어 : </label>
				<input id="F1" class="s_input" name="searchWrd" type="text"  size="35" title="<spring:message code="title.search" /> <spring:message code="input.input" />" value='<c:out value="${searchVO.searchWrd}"/>'  maxlength="155" >
	...
	<tr onClick="fn_egov_inquire_bbsdetail('<c:out value="${resultInfo.bbsId}"/>');return false;">
		<td><c:out value="${(searchVO.pageIndex-1) * searchVO.pageSize + status.count}"/></td>
		<td class="left"><c:out value='${fn:substring(resultInfo.bbsNm, 0, 40)}'/></td>
