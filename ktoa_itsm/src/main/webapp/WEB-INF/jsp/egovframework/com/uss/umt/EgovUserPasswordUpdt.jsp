...
function resetPw(){
	document.passwordChgVO.action = "<c:url value='/uss/umt/EgovUserPasswordReset.do'/>";            
    document.passwordChgVO.submit();
}
...
<div class="wTableFrm">
			...
			<td class="left">
				<c:choose>
					<c:when test="${sessionScope.loginVO.authorCode eq 'ROLE_ADMIN'}">
						<input name="userId" id="userId" title="사용자아이디" type="text" size="20" value="<c:out value='${userManageVO.userId}'/>"  maxlength="20" readonly/>
					...
					<c:when test="${sessionScope.loginVO.authorCode eq 'ROLE_USER'}">
						<input name="userId" id="userId" title="사용자아이디" type="text" size="20" value="<c:out value='${sessionScope.loginVO.id}'/>"  maxlength="20" readonly/>
						<input name="uniqId" id="uniqId" title="uniqId" type="hidden" size="20" value="<c:out value='${sessionScope.loginVO.uniqId}'/>"/>
					</c:when>
				</c:choose>
			</td>
		</tr>
	...
	<c:if test="${sessionScope.loginVO.authorCode eq 'ROLE_ADMIN'}">
		<input type="button" class="btn_submit" onClick="resetPw(); return false;" value="비밀번호 초기화" title="비밀번호 초기화" /><!-- 비밀번호 초기화 -->
	</c:if>
	...
	<input type="button" class="btn_submit" onClick="document.passwordChgVO.reset();return false;" title="<spring:message code="button.init" /> <spring:message code="input.button" />" value="<spring:message code="button.init" />"></input>
	<c:choose>
		<c:when test="${sessionScope.loginVO.authorCode eq 'ROLE_ADMIN'}">
