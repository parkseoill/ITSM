...
<script type="text/javaScript" language="javascript" defer="defer">
	...
	function initLogo(){
		parent.frames["_content"].location.href = "/EgovContent.do";
		parent.frames["_left"].location.reload();
	}

	function changeUserInfo() {
		var getUniqId = '<c:out value='${sessionScope.loginVO.uniqId}'/>';
		parent.frames["_content"].location.href = "/uss/umt/EgovUserSelectUpdtView.do?selectedId="+getUniqId;
		parent.frames["_left"].location.reload(); 
	}
</script>
</head>
...
<div id="header">
	<div class="header_box">
		<h1>
			<a href="#LINK" onclick="initLogo();return false"><img src="<c:url value='/images/egovframework/com/cmm/main/ktoa_logo.png' />" alt="KTOA_ITSM"></a>
		</h1>
		<div style="margin-top:4px;">
			<strong class="top_title_strong"><spring:message code="comCmm.top.title"/>
				<!-- 로그인 사용자명(ID) -->
				<c:if test="${loginVO != null}">
					${loginVO.name}(${loginVO.id})<spring:message code="comCmm.unitContent.15"/>
				</c:if>
			</strong>
		    <span id="sessionInfo">
		    	<c:if test="${loginVO != null}">
			    ...
			    <a class="btn02" href="#"  onclick="logout();return false;"><spring:message code="comCmm.unitContent.3"/></a><!-- 로그아웃 -->
				    <c:if test="${sessionScope.loginVO.authorCode ne ''}">
					    <input type="button" class="btn_submit" onClick=changeUserInfo(); return false;" value="내정보 수정" title="내정보 수정" /><!-- 내정보 수정 -->
				    </c:if>
			    </c:if>
		    </span>
	    </div>
