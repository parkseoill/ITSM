...
<%--
 /**
  * @Class Name : BizReportUpdt.jsp
  * @Description : 업무보고서 수정 화면
  * @Modification Information
  * @
  * @ 수정일               수정자             수정내용
  * @ ----------   --------   ---------------------------
  * @ 2022.08.31   이태신            최초 생성
  *
  *  @author 이태신
  *  @since 2022.08.31
  *  @version 1.0
...
<title><c:out value="${bizReportVO.getBizReportKindNm()}"/> 수정</title>
...
$(document).ready(function () {
	...
	//<!-- 작성자가 아닐 경우 -->
	<c:if test="${sessionScope.loginVO.chrgrNo ne bizReportVO.creatChrgrNo}">
	$("[id^='arrAcrsltText']").attr("disabled", true);
	$("[id^='arrPlnText']").attr("disabled", true);
	</c:if>
});
...
<form:form commandName="bizReportVO" id="bizReportForm" name="bizReportForm" method="post" action="${pageContext.request.contextPath}/rsm/wmr/updateBizReportUpdt.do">
...
<form:hidden path="bizReportId" /><!-- 업무보고서ID -->

<div class="wTableFrm">
	<!-- 타이틀 -->
	<h2><c:out value="${bizReportVO.getBizReportKindNm()}"/> 수정</h2>
	...
	<div class="btn">
		...
		&nbsp;|&nbsp; 
		<input class="s_submit" type="submit" value='<spring:message code="button.update" />' onclick="updateBizReport(); return false;" /><!-- 수정 -->
		</c:if>
