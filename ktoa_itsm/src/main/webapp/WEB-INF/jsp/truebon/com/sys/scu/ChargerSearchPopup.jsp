...
function linkPage(pageNo){
	...
	document.chargerMngForm.action = "<c:url value='/sys/scu/ChargerSearchPopup.do'/>";
...
function getChargerNm(vChrgrNm){
	var parentFrom = parent.document.getElementsByTagName('form');
	parentFrom[0].confrId.value = vChrgrNm;
	parent.$('.ui-dialog-content').dialog('close');
...
<div class="popup">
	<h1>담당자명 목록</h1>
	<form name="chargerMngForm" action ="<c:url value='/sys/scu/ChargerSearchPopup.do' />" method="post">
		...
		<tbody class="ov">
			<%-- 데이터를 없을때 화면에 메세지를 출력해준다 --%>
			 ...
			 <c:forEach var="result" items="${resultList}" varStatus="status">
			  <tr onclick="getChargerNm('<c:out value="${result.chrgrNm}"/>'); return false;">
