...
public class EgovUserManageServiceImpl extends EgovAbstractServiceImpl implements EgovUserManageService {
	...
	public String insertUser(UserManageVO userManageVO) throws Exception {
		...
		userManageVO.setRegstrId((user == null || user.getId() == null) ? "" : user.getId());
		
		//패스워드 암호화
		String pass = EgovFileScrty.encryptPassword(userManageVO.getPassword(), EgovStringUtil.isNullToString(userManageVO.getUserId()));//KISA 보안약점 조치 (2018-10-29, 윤창원)
...
//		userManageVO.setPassword(pass);
		...
		// 로그인 객체 선언
		LoginVO user = (LoginVO)EgovUserDetailsHelper.getAuthenticatedUser();
		userManageVO.setAmndrId((user == null || user.getId() == null) ? "" : user.getId());
